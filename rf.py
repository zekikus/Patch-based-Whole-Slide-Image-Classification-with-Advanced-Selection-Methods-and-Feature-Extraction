...
if __name__ == '__main__':
    ...
    # Predict using RandomForestClassifier model
    ypred = rf_model.predict(test_embeds_flat)
    
    end_time = time.time()  # End time
    print("Execution time of prediction:", end_time - start_time, "seconds")
    
    conf_matris_path= f"ConfMatrix\\rf_{selected_patch_num}patches_not_discarding.png"
    # Compare predictions
    predict(flattened_img_path_list=flattened_img_path_list, ypred=ypred, model="Random Forest", outputh_path = conf_matris_path, selected_patch_num=selected_patch_num)
